...
<div style="background-color: transparent; padding: 10px 0px; margin-bottom: 25px;">
    <h2 style="color: #111111; font-size: 1.8rem; margin-bottom: 15px; font-weight: 700;">Ekspansi Masif: {tot_smelter} Fasilitas Smelter Mengunci Lanskap Ekologis Sulawesi</h2>
    <p style="color: #424242; font-size: 1.05rem; line-height: 1.7; margin-bottom: 15px;">
    ...
    <p style="color: #424242; font-size: 1.05rem; line-height: 1.7;">
...
for i, prov in enumerate(provinces):
    ...
    chart = alt.Chart(df_prov).mark_area(opacity=0.9).encode(
        ...
        title=alt.TitleParams(text=prov, anchor='middle', fontSize=13, color='#111111'),
        height=220
    ).configure_view(
        stroke=None
    ).configure_axis(
...
for i, prov in enumerate(provinces):
    ...
    text_latest = bar_latest.mark_text(
        align='left', baseline='middle', dx=3, color='#111111', fontSize=10
    ).encode(
...
st.markdown(f"""
<div style="background:#F8F9FA; padding:14px; border-radius:10px; border-left:5px solid #D32F2F; margin-bottom: 20px;">
...
with cr2:
    ...
    st.markdown(f"""
    <div style="background:#F8F9FA; padding:14px; border-radius:10px; border-left:5px solid {ord_col_2}; height: 100%;">
...
with cr2_3:
    ...
    st.markdown(f"""
    <div style="background:#F8F9FA; padding:14px; border-radius:10px; border-left:5px solid {ord_col_3}; height: 100%;">
...
for name, lon1, lat1, lon2, lat2, color, offset in MAP_ROUTES:
    ...
    fig_map.add_trace(
        go.Scattergeo(
            ...
            lat=[lat1],
            mode='markers',
            marker=dict(size=6, color=color, line=dict(width=1, color='#111111')),
            name=name,
            text=[hover_text],
            hoverinfo='text',
            showlegend=False
...
st.markdown("""
<div style="background:#F8F9FA; padding:20px; border-radius:10px; border-left:5px solid #D32F2F; margin-top: 20px;">
