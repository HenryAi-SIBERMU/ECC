...
for idx, row in agg_df.iterrows():
    ...
    if status == "Aman":
        kondisi = f"Ringan<br><span style='font-size: 0.85rem; color: #757575;'>({range_str})</span>"
        ...
        kondisi = f"Sedang<br><span style='font-size: 0.85rem; color: #757575;'>({range_str})</span>"
...
st.markdown(f"""
<div style="background-color: #111111; border: 1px solid #E0E0E0; padding: 25px; border-radius: 10px; margin-top: 30px; margin-bottom: 30px;">
...
try:
    ...
    fig_pltu = px.line(df_melt, x='Tahun', y='Capacity (MW)', color='Provinsi', markers=True, title="Timeline Pertumbuhan Kapasitas PLTU Captive (Hingga 2024)", color_discrete_sequence=px.colors.qualitative.Set1)
    fig_pltu.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=40, b=0), height=350, xaxis_title="Tahun", yaxis_title="Kapasitas (MW)")
