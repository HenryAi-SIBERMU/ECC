...
st.markdown('<span style="background:#5C2B6A;color:#E1BEE7;padding:4px 10px;border-radius:5px;font-size:0.85rem;">Metode: Grouped Bar Chart / Disparity Analysis (Data 2022)</span>', unsafe_allow_html=True)
...
pattern = re.compile(r'# ══════════════════════════════════════════════════════════\\n# SUB-BAB 3\.2: KESENJANGAN FASILITAS KESEHATAN.*', re.DOTALL)
...
if match:
    ...
    with open('pages/3_Beban_Kesehatan.py', 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Injeksi Section 3.2 Bar Chart dan Crosstab sukses.")
else:
    print("Gagal regex untuk Section 3.2")
