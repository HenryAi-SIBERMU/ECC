...
class MockStreamlit:
    ...
    def altair_chart(self, chart, **kwargs):
        ...
        path = self.output_dir / f"visuals/chart_full_3_{self.chart_counter}.png"
        try:
            print(f"Saving altair chart {self.chart_counter}")
            chart.save(str(path))
            self.md_lines.append(f"![Chart {self.chart_counter}](visuals/chart_full_3_{self.chart_counter}.png)\n\n")
...
page_path = BASE_DIR / "pages" / "3_Beban_Kesehatan.py"
...
with open(OUT_DIR / "chapter_3_test.md", "w", encoding="utf-8") as f:
    f.write(final_md)
print("Done!")
