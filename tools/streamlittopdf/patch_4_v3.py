...
# Clean up by rewriting the string. Let's find `# Default crosstab table strings` to `out_path = HERE`
pattern = re.compile(r"# Default crosstab table strings.*?(?=out_path = HERE / \"chapter_4.md\")", re.DOTALL)

new_markdown_logic = r"""# Default crosstab table strings
...
# Apply the regex substitution to replace everything from `# Default crosstab table strings` up to `out_path = HERE`
