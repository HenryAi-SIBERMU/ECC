...
target_file = HERE / "extract_chapter_7.py"

with open(target_file, "r", encoding="utf-8") as f:
    content = f.read()

# Define the new markdown block
new_md = r'''md = f"""# Bab 7: Kegagalan Tata Kelola: D3TLH Dalam Sistem Perizinan
...
# Regex to replace everything from `md = f"""# Bab 7...` up to the end of the file.
pattern = re.compile(r'md\s*=\s*f"""# Bab 7: Kegagalan Tata Kelola: D3TLH Dalam Sistem Perizinan.*?^"""', re.MULTILINE | re.DOTALL)
