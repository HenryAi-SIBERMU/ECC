...
class MockContext:
    ...
    def __getattr__(self, name): return lambda *args, **kwargs: MockContext()
...
class StMock:
    ...
    def expander(self, title, *args, **kwargs):
        return MockContext()
        
    def tabs(self, tabs):
        return [MockContext() for _ in range(len(tabs))]
    ...
    def plotly_chart(self, fig, **kwargs):
        path = f"visuals_bab2/chart_exact_{self.img_counter}.png"
        print("Saving", path)
        try:
            fig.write_image(path, width=900, height=450)
            self.markdown_lines.append(f"\n![Chart]({path})\n")
        except Exception as e:
            print("Failed to save plotly:", e)
        self.img_counter += 1
...
<div style="background-color: transparent; padding: 10px 0px; margin-bottom: 25px;">
    <h2 style="color: #FFFFFF; font-size: 1.8rem; margin-bottom: 15px; font-weight: 700;">Terdampak Ekologis: Ketika {tot_smelter} Smelter Mencekik Napas dan Air Sulawesi</h2>
    <p style="color: #CCCCCC; font-size: 1.05rem; line-height: 1.7; margin-bottom: 15px;">
        Pesta "Hilirisasi" nikel ternyata berdampak langsung pada ekosistem Pulau Sulawesi. Konsentrasi <b>{tot_smelter} fasilitas mega-smelter</b> yang ditenagai oleh mesin pembakaran batu bara raksasa sebesar <b>{tot_kapasitas_pltu:,.0f} MW</b> tidak hanya mencaplok ruang spasial, tetapi telah menurunkan indikator daya dukung lingkungan ke titik terendahnya. Di bawah bayang-bayang narasi transisi energi, kualitas hidup jutaan warga dipertaruhkan oleh penurunan masif kualitas air dan udara, ditambah ancaman peningkatan signifikan limbah beracun.
    </p>
...
interp_insig_21 = "Kegagalan statistik mendeteksi signifikansi membongkar fakta krusial: Indeks Kualitas Air (IKA) provinsi adalah metrik usang yang 'mengencerkan' krisis ekologis di tapak. Pencemaran tailing fatal di area tambang tertutupi oleh data sungai-sungai perawan di luarnya."
...
st.markdown(f"""
Tumpukan area berwarna pada grafik di bawah ini merepresentasikan lonjakan kumulatif kapasitas Pembangkit Listrik Tenaga Uap (PLTU) berbasis batu bara (*captive power plants*) yang sengaja didirikan secara khusus untuk menyuplai energi kotor ke fasilitas smelter nikel. Kita melihat peningkatan signifikan kapasitas pembakaran batu bara yang terus meroket secara radikal dan tanpa jeda sepanjang satu dekade terakhir, hingga memuncak di angka masif **{tot_kapasitas_pltu:,.0f} Megawatt (MW)**. Besaran energi kotor ini secara ekuivalen setara dengan daya hancur emisi sulfur dioksida (SO2) dan nitrogen dioksida (NO2) dalam skala mematikan yang tak terbayangkan.
...
Fakta lapangan yang direpresentasikan data satelit ini mengoreksi kampanye artifisial "Hilirisasi Hijau" yang selama ini digaungkan oleh negara dan oligarki korporasi. Melalui angka-angka mutlak dari luar angkasa ini, terbukti bahwa kita tidak sedang memproduksi bahan baku transisi energi yang ramah lingkungan; kita justru sedang mensponsori penciptaan area dengan prevalensi penyakit tinggi saluran pernapasan (ISPA) raksasa di lingkar industri. Asap beracun dari pembakaran batu bara ini mungkin bisa disembunyikan di atas kertas regulasi IKU, tetapi tidak bisa lolos dari pantauan satelit global. Sub-bab ini menguji hipotesis: **Apakah peningkatan signifikan kapasitas PLTU captive secara empiris meruntuhkan kualitas udara?**
""")
...
narasi_ledakan = f"""
Distribusi spasial kapasitas Pembangkit Listrik Tenaga Uap (PLTU) *captive* di Pulau Sulawesi mengungkap realitas ketimpangan infrastruktur energi yang sangat ekstrem, yang membelah pulau ini menjadi dua realitas ekologis yang berbeda. Data secara gamblang menunjukkan bahwa peningkatan signifikan kapasitas energi kotor selama lebih dari satu dekade terakhir tidak terjadi secara merata, melainkan terpusat dan terkonsentrasi secara mutlak di **Daerah Sentra Tambang** (Sulawesi Tengah dan Sulawesi Tenggara). Saat ini, total kapasitas PLTU *captive* yang beroperasi penuh di wilayah sentra tambang telah mencapai angka raksasa sebesar **{max_sentra:,.0f} Megawatt (MW)**, berbanding terbalik dengan Daerah Non-Sentra yang mengalami stagnasi absolut dan hanya mencatatkan kapasitas marjinal sebesar **{max_non_sentra:,.0f} MW**.

Angka-angka ini bukan sekadar statistik di atas kertas; mereka adalah bukti empiris dari pembentukan "Zona Terdampak" (*sacrifice zones*) berskala masif. Dengan proporsi dominasi yang mencapai angka fantastis **{pct_sentra:.1f}%** dari total kapasitas pembangkit kotor di seluruh wilayah, dua provinsi sentra nikel ini telah secara sistematis dan terstruktur diubah menjadi episentrum pembuangan limbah udara mematikan. Lonjakan eksponensial yang tergambar jelas dari garis tren berwarna merah (berbanding dengan garis abu-abu yang datar) secara empiris mematahkan narasi "pembangunan inklusif" atau "hilirisasi hijau" yang sering digemakan oleh aktor negara dan korporasi. 

Alih-alih mendistribusikan kesejahteraan ekonomi ke seluruh penjuru pulau, model ekspansi hilirisasi nikel justru memonopoli alokasi energi mematikan ini secara eksklusif ke wilayah-wilayah lingkar tambang dan smelter, mengorbankan kualitas hidup, kesehatan paru-paru, dan ruang udara segar masyarakat lokal demi melayani rakusnya rantai pasok industri global. Fakta ketimpangan **{max_sentra:,.0f} MW berbanding {max_non_sentra:,.0f} MW** ini secara meyakinkan menegaskan satu kesimpulan kausalitas yang tak terbantahkan: peningkatan signifikan industri ekstraktif adalah variabel tunggal yang bertanggung jawab memicu invasi infrastruktur energi berbasis fosil terbesar di Indonesia bagian timur, memaksa penduduk di wilayah sentra untuk menghirup debu pembakaran batu bara setiap detik demi subsidi gaya hidup transisi energi di negara-negara maju.
"""
...
md_out = "\n".join(st.markdown_lines)
with open("chapter_2_exact.md", "w", encoding="utf-8") as f:
    f.write(md_out)
print("Saved chapter_2_exact.md!")
