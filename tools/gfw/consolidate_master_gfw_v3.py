...
def main():
    ...
    # 1. Tree Cover Loss
    f_loss = get_file("mega_fetch_v3", "tree_cover_loss_*.csv")
    if f_loss:
    ...
    f_primary = get_file("mega_fetch_v3", "primary_forest_loss_*v3.csv")
    if f_primary:
        df = pd.read_csv(f_primary)
        # BUG FIX: Filter HANYA Hutan Primer (True) sebelum di-sum
    ...
    f_pa = get_file("mega_fetch_v3", "loss_in_protected_areas_*v3.csv")
    if f_pa:
        df = pd.read_csv(f_pa)
        # BUG FIX: Hapus kategori 0 (Di luar kawasan lindung)
    ...
    f_driver = get_file("land_api_fetch", "loss_by_driver_*v3.csv")
    if f_driver:
    ...
    out_path = os.path.join(out_dir, 'sulawesi_gfw_master_1_dekade_2014_2023_v3.csv')
