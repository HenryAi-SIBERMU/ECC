...
def clean_loss_by_driver(input_dir, output_dir):
    pattern = os.path.join(input_dir, "land_api_fetch", "loss_by_driver_*v3.csv")
    ...
    out_path = os.path.join(output_dir, "sulawesi_gfw_loss_by_driver_2014_2023_v3.csv")
    df.to_csv(out_path, index=False)
    print(f"Berhasil Disimpan: {out_path} ({len(df)} baris)")

def clean_primary_forest_loss(input_dir, output_dir):
    pattern = os.path.join(input_dir, "mega_fetch_v3", "primary_forest_loss_*v3.csv")
    files = glob.glob(pattern)
    if not files:
    ...
    # BUG FIX: HANYA ambil Hutan Primer sungguhan
    if 'is__umd_regional_primary_forest_2001' in df.columns:
        df = df[df['is__umd_regional_primary_forest_2001'] == True]
    ...
    out_path = os.path.join(output_dir, "sulawesi_gfw_hutan_primer_loss_2014_2023_v3.csv")
    df.to_csv(out_path, index=False)
    print(f"Berhasil Disimpan: {out_path} ({len(df)} baris)")

def clean_protected_areas_loss(input_dir, output_dir):
    pattern = os.path.join(input_dir, "mega_fetch_v3", "loss_in_protected_areas_*v3.csv")
    files = glob.glob(pattern)
    if not files:
    ...
    # BUG FIX: Hapus area yang bukan kawasan lindung (0)
    if 'wdpa_protected_areas__iucn_cat' in df.columns:
        df = df[df['wdpa_protected_areas__iucn_cat'] != 0]
    ...
    out_path = os.path.join(output_dir, "sulawesi_gfw_kawasan_lindung_loss_2014_2023_v3.csv")
    df.to_csv(out_path, index=False)
    print(f"Berhasil Disimpan: {out_path} ({len(df)} baris)")
