...
def create_driver_analysis(province_name, admin_code, canopy_cover=30):
    ...
    try:
        ...
        if response.status_code == 202:
            ...
            import re
            # Use strict 36-char UUID matching to bypass GFW's bug where they prepend an 'f' in the 409 error message
            match = re.search(r'([a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12})', message)
            if match:
                resource_id = match.group(1)
                print(f"  ℹ️ Found existing Resource ID from 409: {resource_id}")
                return resource_id, None
            print(f"  ❌ 409 Error but couldn't parse ID: {message}")
            return None, None
        else:
...
def main():
    ...
    if not resource_ids:
        print("\n⚠️ Warning: No resource IDs collected. API might be returning 409 if already cached, script needs to parse 409 response to get existing ID.")
    ...
    if all_results:
        ...
        output_file = output_dir / "loss_by_driver_sulawesi_2001_2025_v3.csv"
