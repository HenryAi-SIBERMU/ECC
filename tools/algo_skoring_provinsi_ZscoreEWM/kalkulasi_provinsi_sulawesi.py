...
def load_data():
    ...
    df_konflik = load_csv("sulawesi_konflik_agraria_tanahkita_v2.csv")
    ...
    df_gfw = load_csv("sulawesi_gfw_master_1_dekade_2014_2023_v3.csv")
    df_gfw_lindung = load_csv("sulawesi_gfw_kawasan_lindung_loss_2014_2023_v3.csv")
    df_gfw_driver = load_csv("sulawesi_gfw_loss_by_driver_2014_2023_v3.csv")
...
def build_raw_data():
    ...
    for prov in PROVINSI_LIST:
        metrics = {
            ...
            'kepadatan_iup': 0.0, 'fpic': 0, 'jiwa_terdampak': 0, 'kriminalisasi': 0, 
        ...
        if not df_nasa.empty:
            ...
            if not df_kes.empty:
                pop_prov = populasi_bps.get(prov, 1)
                pop_non = sum([populasi_bps.get(p, 1) for p in populasi_bps if p != prov])
              
                df_ispa = df_kes[df_kes['indikator'].str.contains('ISPA', case=False, na=False)]
                val_ispa = df_ispa[df_ispa['provinsi'] == prov]['nilai'].sum()
                sum_non_ispa = df_ispa[df_ispa['provinsi'] != prov]['nilai'].sum()
              
                ir_prov_ispa = (val_ispa / pop_prov) * 10000
                ir_non_ispa = (sum_non_ispa / pop_non) * 10000 if pop_non > 0 else 1
                metrics['ispa_irr'] = float(ir_prov_ispa / ir_non_ispa if ir_non_ispa > 0 else 0)
                ...
                ir_prov_diare = (val_diare / pop_prov) * 10000
                ir_non_diare = (sum_non_diare / pop_non) * 10000 if pop_non > 0 else 1
                metrics['diare_irr'] = float(ir_prov_diare / ir_non_diare if ir_non_diare > 0 else 0)
        ...
        if not df_b3.empty:
            ...
            metrics['b3_ton'] = float(df_b3_clean[df_b3_clean['Provinsi'] == prov]['Estimasi Timbulan (Ton/Tahun)'].sum() / 1_000_000.0)
            
            # Filter khusus Tailing/Slag/DSTP untuk Air 4
            df_tailing = df_b3_clean[df_b3_clean['Jenis Limbah B3'].str.contains('tailing|slag|dstp', case=False, na=False)]
            metrics['tailing_ton'] = float(df_tailing[df_tailing['Provinsi'] == prov]['Estimasi Timbulan (Ton/Tahun)'].sum() / 1_000_000.0)
        ...
        if not df_konflik.empty:
            ...
            if 'tahun' in df_konflik.columns:
                ...
                df_konflik_recent = df_konflik
            
            if 'provinsi_ner' in df_konflik_recent.columns:
                df_konf_prov = df_konflik_recent[df_konflik_recent['provinsi_ner'] == prov]
            else:
                prov_keyword = prov.split()[-1]
                df_konf_prov = df_konflik_recent[df_konflik_recent['lokasi'].str.contains(prov_keyword, case=False, na=False) | df_konflik_recent['judul'].str.contains(prov_keyword, case=False, na=False)]
        ...
        if not df_kawasan_nikel.empty:
            ...
            if not sentra_kn.empty:
                ...
                metrics['kepadatan_iup'] = float(tiup / luas_daratan)
                
        if not df_konflik_fpic.empty:
            df_konflik_fpic['tahun'] = pd.to_numeric(df_konflik_fpic['tahun'], errors='coerce')
            df_fpic_recent = df_konflik_fpic[df_konflik_fpic['tahun'] >= 2014]
            metrics['fpic'] = len(df_fpic_recent[(df_fpic_recent['provinsi'].str.contains(prov.split()[-1], case=False, na=False)) & (df_fpic_recent['indikasi_fpic'] == True)])
        ...
        if not df_izin.empty:
            ...
            metrics['izin_baru'] = int(df_izin_recent['Jumlah_Izin_Baru'].sum()) if 'Jumlah_Izin_Baru' in df_izin_recent.columns else 0
        ...
        # Tailing (proxy dihapus karena menimbulkan impresi duplikasi bug di UI, menunggu data sesungguhnya)
        # metrics['tailing_ton'] = 0.0
        
        # Defisit Faskes SPA (Sesuai Target RPJMN 80%)
        # Dinamis: Diambil dari raw_kemenkes_puskesmas_2024.csv Baris 39
        parsed_spa = {
            'Gorontalo': 94.12,
        ...
        spa_aktual = parsed_spa.get(prov, 60.0)
            
        metrics['gap_spa'] = float(max(0.0, 80.0 - spa_aktual))
...
def kalkulasi_skor_provinsi_sulawesi(df_prov_data=None):
    ...
    matriks_columns = {
        ...
        'Lahan': ['bencana', 'deforestasi_ha', 'lindung_ha', 'driver_ha', 'kepadatan_iup'],
    ...
    for prov in PROVINSI_LIST:
        ...
        # ============================================================
        # OPSI B: Agregasi EWM Weighted Average (Bobot EWM Aktif)
        # Indikator yang lebih timpang antar provinsi = bobot lebih besar
        # ============================================================
        def ewm_weighted_avg(cols):
            valid_cols = [c for c in cols if c in prov_likert.index and c in ewm_weights.index]
            if not valid_cols:
                return 0.0
            w = ewm_weights[valid_cols]
            w_sum = w.sum()
            if w_sum == 0:
                return prov_likert[valid_cols].mean()
            return (prov_likert[valid_cols] * w).sum() / w_sum

        skor_u = ewm_weighted_avg(matriks_columns['Udara'])
        skor_a = ewm_weighted_avg(matriks_columns['Air'])
        skor_l = ewm_weighted_avg(matriks_columns['Lahan'])
        skor_s = ewm_weighted_avg(matriks_columns['Sosial'])
        skor_v = ewm_weighted_avg(matriks_columns['Veto'])
        ...
        results_by_prov[prov] = {
            ...
            "math_details": {
                "mean": means.to_dict(),
                "std": stds.to_dict(),
                "ewm_weights": ewm_weights.to_dict(),
                "likert": prov_likert.to_dict()
            }
        }
