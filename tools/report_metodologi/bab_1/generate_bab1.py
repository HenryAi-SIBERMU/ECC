...
def generate_all_bab1():
    ...
    add_p(doc, [
        ("TL;DR (Ringkasan Eksekutif): ", True, False),
    ...
    add_p(doc, [
        ...
        (f"Nilai {test_name} sebesar {test_val} dengan ({dof_str}) menghasilkan nilai p {fmt_p(p_val_display)} (di bawah batas kritis alpha 0.05). Hipotesis Nol (H0) ditolak secara mutlak: terbukti secara empiris bahwa penambahan kapasitas PLTU Captive berkorelasi langsung dengan lonjakan kehilangan tutupan hutan.\n", False, False),
        ("2. ", True, False), (f"Kelipatan Risiko Bencana Ekologis (Odds Ratio = {or_pltu:.2f}x): ", True, False),
        (f"Nilai Odds Ratio sebesar {or_pltu:.2f}x membuktikan bahwa wilayah dan periode tahun yang mengoperasikan PLTU Captive menghadapi peluang mengalami deforestasi komoditas tinggi sebesar {int(or_pltu)} KALI LIPAT lebih besar dibandingkan wilayah tanpa PLTU Captive. Infrastruktur pembangkit batu bara tidak hanya menghasilkan polusi cerobong, tetapi memicu konversi hutan masif untuk penimbunan batu bara (coal yard), jalur transmisi listrik privat, jalan angkut logistik (haul road), dan dermaga curah khusus (jetty).\n", False, False),
    ...
    add_p(doc, [
        ...
        ("Peningkatan tajam penerbitan IUP di kawasan timur Sulawesi berbanding lurus dengan perluasan area konversi hutan. Konsentrasi perizinan yang selaras dengan pusat pengembangan hilirisasi nikel menunjukkan mutlaknya instrumen tata ruang dan evaluasi lingkungan untuk ditegakkan secara ketat.", False, False)
    ])
    ...
    add_h4(doc, "D. Matriks Hasil Uji Empiris: Konsentrasi Spasial & Skenario Crosstab")
    add_p(doc, [
        ("Akumulasi realisasi penanaman modal dan hubungannya dengan eskalasi kehilangan tutupan hutan di masing-masing provinsi dapat dilihat secara empiris pada ", False, False),
        ("Tabel 1.7b", True, False),
        (" berikut:", False, False),
    ])

    prov_list_1_4 = ["Sulawesi Tengah", "Sulawesi Tenggara", "Sulawesi Selatan", "Sulawesi Utara", "Gorontalo", "Sulawesi Barat"]
    emp_rows_1_4 = []
    html_emp_rows_1_4 = ""
    md_emp_rows_1_4 = ""
    for idx, prov in enumerate(prov_list_1_4):
        inv_juta = float(df_panel_1_4[df_panel_1_4['Provinsi'] == prov]['Investasi_Juta_Rp'].sum())
        inv_triliun = inv_juta / 1_000_000
        def_ha = float(df_panel_1_4[df_panel_1_4['Provinsi'] == prov]['Deforestasi_Driver_Komoditas_Tambang_Sawit_Ha'].sum())
        
        kat = "Sentra Ekstraktif Utama" if prov in ["Sulawesi Tengah", "Sulawesi Tenggara", "Sulawesi Selatan"] else "Area Non-Smelter"
        
        emp_rows_1_4.append([prov, f"Rp {inv_triliun:,.2f} Triliun", f"{def_ha:,.1f} Ha", kat])
        row_cls = "data-tr-even" if (idx + 1) % 2 == 0 else "data-tr-odd"
        html_emp_rows_1_4 += f"    <tr class=\"{row_cls}\"><td class=\"data-td\">{prov}</td><td class=\"data-td\" style=\"text-align:center;\">Rp {inv_triliun:,.2f} Triliun</td><td class=\"data-td\" style=\"text-align:center;\">{def_ha:,.1f} Ha</td><td class=\"data-td\">{kat}</td></tr>\n"
        md_emp_rows_1_4 += f"| {prov} | Rp {inv_triliun:,.2f} Triliun | {def_ha:,.1f} Ha | {kat} |\n"

    add_caption(doc, "Tabel 1.7b: Rincian Spasial Realisasi Investasi PMDN dan Deforestasi Komoditas 6 Provinsi (Dekade 2014-2024)")
    emp_headers_1_4 = ["Provinsi", "Akumulasi PMDN", "Deforestasi Komoditas", "Kategori Spasial"]
    add_table_1col(doc, emp_headers_1_4, emp_rows_1_4, [3.8, 3.5, 4.0, 3.5], ['L', 'C', 'C', 'L'])
    ...
    add_h4(doc, "B. Alur Logika Metodologis (Validasi Silang & Kurva Bézier)")
    add_p(doc, [
        ("Verifikasi titik pelabuhan dan terminal khusus ekspor nikel dilakukan melalui protokol triangulasi informasi publik:", False, False),
    ])
    add_p(doc, [
    ...
    add_h4(doc, "D. Matriks Hasil Uji Empiris: Inventarisasi 6 Simpul Pelabuhan")
    add_p(doc, [
    ...
    log_rows = [
        ["Kawasan Industri Morowali (IMIP)", "Morowali, Sulawesi Tengah", "Pelabuhan Samudera & Dermaga Curah", "PSN (Perpres 109/2020)", "Hingga 52.378 DWT", "Pasar Global (Tiongkok)"],
        ["Kawasan Industri Morowali Utara (GNI)", "Morowali Utara, Sulawesi Tengah", "Terminal Khusus Pesisir Teluk Tomori", "Izin Industri Mandiri", "Hingga 30.000 DWT", "Pasar Global (Tiongkok)"],
        ...
        ["Kawasan Industri Sorowako (Vale)", "Luwu Timur, Sulawesi Selatan", "Pelabuhan Khusus Balantang Malili", "Kontrak Karya Tambang", "Hingga 15.000 DWT", "Jepang & Skandinavia"],
    ]
    add_table_1col(doc, log_headers, log_rows, [3.2, 3.2, 3.8, 2.8, 2.0, 3.0], ['L', 'L', 'L', 'C', 'C', 'L'])

    add_h4(doc, "E. Interpretasi Spasial Industri: Rantai Pasok Global")
    add_p(doc, [
...
<p>
  <strong>TL;DR (Ringkasan Eksekutif):</strong> Konsentrasi masif fasilitas smelter dan 9.825 MW PLTU Captive di Sulawesi Tengah, Tenggara, dan Selatan terbukti memicu ledakan deforestasi. Uji Chi-Square membuktikan secara absolut bahwa daerah dengan PLTU Captive memiliki risiko deforestasi 18 kali lipat lebih parah dibandingkan wilayah non-smelter. Daya rusak industri ini melampaui tapak proyek (spillover effect).
</p>
...
<ul style="margin-left: 20px; margin-bottom: 15px;">
  <li style="margin-bottom: 6px;"><strong>1. Pemusatan Ekstrem di 3 Sentra Ekstraktif Utama:</strong> Tabel 1.4b mengonfirmasi bahwa 100% kapasitas PLTU Captive (9.825 MW) dan mayoritas fasilitas smelter (717 unit) berpusat secara absolut di Sulawesi Tengah, Tenggara, dan Selatan. Pemusatan spasial ini berbanding lurus dengan akumulasi deforestasi komoditas terparah di ketiga wilayah tersebut yang menembus ratusan ribu hektar, berbanding terbalik dengan 3 provinsi "Area Non-Smelter" lainnya.</li>
  <li style="margin-bottom: 6px;"><strong>2. Signifikansi Statistik yang Sangat Kuat (p {fmt_p(p_val_display)}):</strong> Nilai {test_name} sebesar {test_val} dengan ({dof_str}) menghasilkan nilai p {fmt_p(p_val_display)} (di bawah batas kritis alpha 0.05). Hipotesis Nol (H0) ditolak: terbukti secara empiris bahwa penambahan kapasitas PLTU Captive berkorelasi langsung dengan lonjakan kehilangan tutupan hutan.</li>
  <li style="margin-bottom: 6px;"><strong>3. Kelipatan Risiko Bencana Ekologis (Odds Ratio = {or_pltu:.2f}x):</strong> Nilai Odds Ratio sebesar {or_pltu:.2f}x membuktikan bahwa wilayah dan periode tahun yang mengoperasikan PLTU Captive menghadapi peluang mengalami deforestasi komoditas tinggi sebesar {int(or_pltu)} KALI LIPAT lebih besar dibandingkan wilayah tanpa PLTU Captive. Infrastruktur pembangkit batu bara tidak hanya menghasilkan polusi cerobong, tetapi memicu konversi hutan masif untuk penimbunan batu bara (coal yard), jalur transmisi listrik privat, jalan angkut logistik (haul road), dan dermaga curah khusus (jetty).</li>
  <li style="margin-bottom: 6px;"><strong>4. Efek Meluber Lintas Batas (Spillover Effect) & Emisi Karbon Terkunci:</strong> Meskipun tapak pembangkit berada di kawasan pesisir tertentu (seperti Morowali dan Konawe), eksternalitas destruktifnya merambat jauh melampaui batas administratif proyek (<em>spillover effect</em>). Pengoperasian 9.825 MW PLTU captive mengunci ketergantungan bahan bakar batu bara puluhan juta ton per tahun, mendegradasi tutupan daerah aliran sungai (DAS), mencemari ekosistem perairan laut, dan mengorbankan ruang hidup masyarakat lokal secara permanen.</li>
</ul>
...
<h4>D. Matriks Hasil Uji Empiris: Konsentrasi Spasial & Skenario Crosstab</h4>
<p>
  Akumulasi realisasi penanaman modal dan hubungannya dengan eskalasi kehilangan tutupan hutan di masing-masing provinsi dapat dilihat secara empiris pada <strong>Tabel 1.7b</strong> berikut:
</p>
<div class="table-caption">Tabel 1.7b: Rincian Spasial Realisasi Investasi PMDN dan Deforestasi Komoditas 6 Provinsi (Dekade 2014-2024)</div>
<table class="data-table">
  <thead>
    <tr>
      <th class="data-th">Provinsi</th>
      <th class="data-th">Akumulasi PMDN</th>
      <th class="data-th">Deforestasi Komoditas</th>
      <th class="data-th">Kategori Spasial</th>
    </tr>
  </thead>
  <tbody>
...
"""
    ...
    md_lines.extend([
        ...
        "",
        f"1. **Signifikansi Statistik yang Sangat Kuat (p {fmt_p(p_val_display)}):** Nilai {test_name} sebesar {test_val} dengan ({dof_str}) menghasilkan nilai p {fmt_p(p_val_display)} (di bawah batas kritis alpha 0.05). Hipotesis Nol (H0) ditolak: terbukti secara empiris bahwa penambahan kapasitas PLTU Captive berkorelasi langsung dengan lonjakan kehilangan tutupan hutan.",
        f"3. **Kelipatan Risiko Bencana Ekologis (Odds Ratio = {or_pltu:.2f}x):** Nilai Odds Ratio sebesar {or_pltu:.2f}x membuktikan bahwa wilayah dan periode tahun yang mengoperasikan PLTU Captive menghadapi peluang mengalami deforestasi komoditas tinggi sebesar {int(or_pltu)} KALI LIPAT lebih besar dibandingkan wilayah tanpa PLTU Captive. Infrastruktur pembangkit batu bara tidak hanya menghasilkan polusi cerobong, tetapi memicu konversi hutan masif untuk penimbunan batu bara (coal yard), jalur transmisi listrik privat, jalan angkut logistik (haul road), dan dermaga curah khusus (jetty).",
        "4. **Efek Meluber Lintas Batas (Spillover Effect) & Emisi Karbon Terkunci:** Meskipun tapak pembangkit berada di kawasan pesisir tertentu (seperti Morowali dan Konawe), eksternalitas destruktifnya merambat jauh melampaui batas administratif proyek (*spillover effect*). Pengoperasian 9.825 MW PLTU captive mengunci ketergantungan bahan bakar batu bara puluhan juta ton per tahun, mendegradasi tutupan daerah aliran sungai (DAS), mencemari ekosistem perairan laut, dan mengorbankan ruang hidup masyarakat lokal secara permanen.",
        "",
        "---",
        "",
...
<table>
  ...
  <tbody>
    ...
    <tr class="data-tr-even"><td class="data-td"><strong>Fase IV: Pemetaan Rantai Pasok</strong></td><td class="data-td">Triangulasi data logistik dan pemodelan maritim</td><td class="data-td">Laporan KNKT, Perpres PSN, Kurva Parametrik Bézier</td><td class="data-td">Peta Alur Rantai Pasok Ekspor & Konsentrasi Spasial 78%</td></tr>
        ...
        "",
        "#### D. Matriks Hasil Uji Empiris: Konsentrasi Spasial & Skenario Crosstab",
        "Akumulasi realisasi penanaman modal dan hubungannya dengan eskalasi kehilangan tutupan hutan di masing-masing provinsi dapat dilihat secara empiris pada **Tabel 1.7b** berikut:",
        "",
        "##### Tabel 1.7b: Rincian Spasial Realisasi Investasi PMDN dan Deforestasi Komoditas 6 Provinsi (Dekade 2014-2024)",
        "| Provinsi | Akumulasi PMDN | Deforestasi Komoditas | Kategori Spasial |",
        "| :--- | :---: | :---: | :--- |",
        ...
        "Verifikasi titik pelabuhan dan terminal khusus ekspor nikel dilakukan melalui protokol triangulasi informasi publik:",
        "",
