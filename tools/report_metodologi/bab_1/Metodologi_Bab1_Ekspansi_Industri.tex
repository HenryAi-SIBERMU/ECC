...
Tabel Asumsi Dasar di bawah gambar. \#\#\#\#\# Bagan Alur 1.5: Alur
Logika Tren Pertumbuhan Izin Tambang Baru
...
Decision Rule (Alpha 5\%) & Jika P-Value \textless{} 0.05, maka Tolak H0
(terbukti signifikan bahwa ekspansi perizinan mendorong deforestasi). \\
Threshold Kategori & Nilai Median Data Panel (N=60): X ≥ 2.0 izin; Y ≥
10,961.8 Ha. \\
Orientasi Odds Ratio & OR = ( a × d ) / ( b × c ) dengan a = Izin Tinggi
\& Deforestasi Tinggi; mengukur risiko deforestasi tinggi pada kelompok
penerbitan izin tinggi. \\
\end{longtable}
}
...
\textbf{Persamaan Laju Pertumbuhan Izin Tahunan (Regresi Komparatif
YoY):}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Pertumbuhan\_Izin (\%) = [ ( Jumlah\_Izin\_t {-} Jumlah\_Izin\_t{-}1 ) / Jumlah\_Izin\_t{-}1 ] × 100}
\end{Highlighting}
\end{Shaded}
...
(atau luasan) pada tahun berjalan (t). - \texttt{Jumlah\_Izin\_t-1}:
Agregasi jumlah izin (atau luasan) pada satu tahun sebelumnya (t - 1).

\textbf{Persamaan Pengklasifikasian Kategori Data (Binning Threshold
Median, Fungsi Piecewise):}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Kategori(x) = \textquotesingle{}Tinggi\textquotesingle{} , jika x ≥ Median(Panel)   |   \textquotesingle{}Rendah\textquotesingle{} , jika x \textless{} Median(Panel)}
\end{Highlighting}
\end{Shaded}

\emph{Keterangan Variabel:} - \texttt{Kategori(x)}: Data panel
...
Penggerak (Driver)}
\texttt{Total\_Deforestasi\_Driver\_k\ =\ Σ\ (\ Area\_Loss\_i\ )\ \ \ ;\ \ \ untuk\ seluruh\ piksel\ i\ dalam\ Kategori\_Driver\_k}
...
Forest Loss)}
\texttt{Total\_Primary\_Loss\ =\ Σ\ (\ Area\_Loss\_j\ )\ \ \ ;\ \ \ untuk\ seluruh\ piksel\ j\ dengan\ Tipe\_Hutan\ =\ \textquotesingle{}Primer\textquotesingle{}}
...
\texttt{Emisi\_CO2\_Total\ =\ Σ\ (\ Area\_Loss\_c\ ×\ Faktor\_Emisi\_Biomassa\_c\ )}
...
\textbf{Persamaan Kategorisasi Nilai Ambang Batas Median (Fungsi
Piecewise):}
...
\subparagraph{Tabel 1.7b: Representasi Spasial Luas Konsesi Baru dan
Deforestasi
(2014-2023)}\label{tabel-1.7b-representasi-spasial-luas-konsesi-baru-dan-deforestasi-2014-2023}
...
Tahun
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
Deforestasi Komoditas (Ha)
\end{minipage} & \begin{minipage}[b]{\linewidth}\centering
Luas Konsesi Tambang Baru (Ha)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
Decision Rule (Alpha 5\%) & Jika P-Value \textless{} 0.05, maka Tolak H0
(terbukti signifikan bahwa realisasi investasi mendorong
deforestasi). \\
Threshold Kategori & Nilai Median Data Panel (N=48): X \textgreater{}
3,146.4 Juta Rp; Y ≥ 10,451.7 Ha. \\
Orientasi Odds Ratio & OR = ( a × d ) / ( b × c ) dengan a = Investasi
Tinggi \& Deforestasi Tinggi; mengukur risiko deforestasi tinggi pada
kelompok realisasi investasi tinggi. \\
\end{longtable}
}
...
2.083 & p = 0.1489 & 2.8 & TIDAK SIGNIFIKAN \\
Realisasi Investasi PMDN (Juta Rp) & Deforestasi Komoditas Tambang/Sawit
(Hektar) & 2.083 & p = 0.1489 & 2.8 & TIDAK SIGNIFIKAN \\
\end{longtable}
}
...
\paragraph{C. Matriks Hasil Uji Empiris: Inventarisasi 6 Simpul
Pelabuhan}\label{c.-matriks-hasil-uji-empiris-inventarisasi-6-simpul-pelabuhan}
...
\paragraph{A. Pengantar \& Kerangka
Narasi}\label{a.-pengantar-kerangka-narasi-3}

Metode analisis pada tahapan ini difokuskan pada \textbf{Pemetaan
Kausalitas (Spasial)} untuk membedah asimetri penguasaan ruang antara
wilayah hulu (origin: sumber ekstraksi di Sulawesi) dan hilir
(destination: pusat industrialisasi luar negeri). Garis diplot
menggunakan rute pelayaran untuk merepresentasikan jarak tempuh aktual
kapal logistik di permukaan bumi.

\paragraph{B. Alur Logika Metodologis (Flowchart Analisis
Spasial)}\label{b.-alur-logika-metodologis-flowchart-analisis-spasial}

Rangkaian proses ekstraksi koordinat hingga visualisasi spasial
direpresentasikan pada \textbf{Bagan Alur 1.6} berikut:

\subparagraph{Bagan Alur 1.6: Alur Logika Metodologis Pemetaan Jalur
Logistik Maritim (Bézier
Curve)}\label{bagan-alur-1.6-alur-logika-metodologis-pemetaan-jalur-logistik-maritim-buxe9zier-curve}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{flowchart LR}
\NormalTok{    A["Ekstraksi Koordinat\textless{}br/\textgreater{}Origin (Sulawesi)"] {-}{-}\textgreater{} B["Penentuan Titik\textless{}br/\textgreater{}Tujuan (China/Jepang)"]}
\NormalTok{    B {-}{-}\textgreater{} C["Kalkulasi Kurva\textless{}br/\textgreater{}Parametrik (Bézier)"]}
\NormalTok{    C {-}{-}\textgreater{} D["Plotting Spasial\textless{}br/\textgreater{}(Plotly Scattergeo)"]}
\NormalTok{    D {-}{-}\textgreater{} E["Peta Jalur Distribusi\textless{}br/\textgreater{}Logistik Maritim"]}
\end{Highlighting}
\end{Shaded}
...
\paragraph{D. Matriks Hasil Pemetaan
Spasial}\label{d.-matriks-hasil-pemetaan-spasial}

Agregasi titik asal (Origin) dan tujuan akhir (Destination) di-render
langsung ke dalam proyeksi spasial. Struktur data logistik yang
menggerakkan pemetaan ini memetakan pelabuhan asal menuju pasar utama,
yaitu \textbf{Tiongkok, Jepang, dan Korea Selatan}.

\paragraph{E. Interpretasi Spasial Industri (Anatomi Rantai
Pasok)}\label{e.-interpretasi-spasial-industri-anatomi-rantai-pasok}

Peta rute logistik maritim mengilustrasikan alur distribusi produk
olahan nikel dari kawasan industri di Sulawesi:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Orientasi Ekspor:} Kawasan industri utama yang berstatus
  Proyek Strategis Nasional (PSN) mengalirkan produk olahan ke
  sentra-sentra industri manufaktur di pasar internasional.
\item
  \textbf{Integrasi Rantai Pasok:} Mayoritas rute pengapalan terhubung
  langsung dengan pelabuhan ekspor tujuan, yang mengindikasikan posisi
  kawasan pemurnian di Sulawesi sebagai pemasok bahan baku setengah
  jadi.
\item
  \textbf{Dinamika Rute Maritim:} Peta rute mencerminkan diversifikasi
  pasar ekspor (Asia Timur) dan jaringan logistik kawasan.
\end{enumerate}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{1.7 Matriks Indikator dan Sumber Data Resmi Bab
...
Unit Izin & 2014-2024 & Data Registry ESDM MODI (Minerbaone) &
\texttt{sulawesi\_izin\_baru\_per\_tahun.csv} \\
2 & Luas Wilayah Konsesi Tambang Baru & Faktor Tekanan Ekstraktif &
Hektar (Ha) & 2014-2024 & Data Registry ESDM MODI (Minerbaone) &
\texttt{sulawesi\_kawasan\_nikel\_luas.csv} \\
3 & Kapasitas Terpasang PLTU Captive & Infrastruktur Energi Khusus &
Megawatt (MW) & 2014-2024 & NGO (Global Energy Monitor / GEM) &
\texttt{sulawesi\_pltu\_captive.csv} \\
4 & Fasilitas Smelter Nikel & Fasilitas Industri Hilir & Unit Fasilitas
& 2014-2024 & Database Smelter CGS \& ESDM MODI &
\texttt{sulawesi\_esdm\_nikel.csv} \\
5 & Realisasi Investasi PMDN \& Nikel & Arus Modal Domestik & Triliun
Rupiah & 2016-2024 & API BPS \& BKPM &
\texttt{sulawesi\_investasi\_pmdn\_2016\_2024.csv} \\
6 & PDRB Provinsi (Ekstraktif vs Akar Rumput) & Struktur Ekonomi Makro &
Triliun Rupiah & 2016-2024 & API BPS (Subject 52) &
\texttt{sulawesi\_pdrb\_sektoral\_2016\_2024.csv} \\
7 & PDRB Kabupaten Sentra Tambang & Struktur Ekonomi Daerah & Triliun
Rupiah & 2016-2024 & API BPS (Subject 52) &
\texttt{sulawesi\_pdrb\_sektoral\_kabupaten\_2016\_2024.csv} \\
8 & Pendapatan Asli Daerah (PAD) \& Breakdown Pajak & Kapasitas Fiskal
Daerah & Triliun Rupiah & 2016-2024 & API BPS &
\texttt{sulawesi\_pad\_breakdown\_2016\_2024.csv} \\
9 & Luas Total Deforestasi Alam \& Komoditas & Dampak Ekologis & Hektar
(Ha) & 2014-2023 & Global Forest Watch (GFW API) &
\texttt{sulawesi\_gfw\_master\_1\_dekade\_2014\_2023\_v3.csv} \\
10 & Simpul Pelabuhan \& Terminal Logistik & Infrastruktur Rantai Pasok
& Titik Koordinat \& DWT & 2014-2024 & Laporan Publik (KNKT, Perpres
PSN, Korporasi) & \texttt{sulawesi\_logistik\_simpul\_nikel.csv} \\
\end{longtable}
}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\subsection{1.8 Bagan Alur Kerangka Kerja Riset Bab
