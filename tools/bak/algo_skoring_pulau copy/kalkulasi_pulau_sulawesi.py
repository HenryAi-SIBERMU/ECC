...
def kalkulasi_skor_pulau_sulawesi(data_empiris):
    ...
    bencana = data_empiris.get('jumlah_bencana', 0)
    skor_lahan_1 = min(10.0, (bencana / 877.0) * 10.0)
    
    deforestasi_ha = data_empiris.get('deforestasi_ha', 0.0)
    skor_lahan_2 = min(10.0, (deforestasi_ha / 638_000.0) * 10.0)
    
    # 3C. Hutan Lindung (Nol Toleransi Hukum / Absolute Zero)
    deforestasi_lindung = data_empiris.get('deforestasi_hutan_lindung_ha', 0.0)
    skor_lahan_3 = 10.0 if deforestasi_lindung > 0 else 0.0
    
    # 3D. Ekstraktif sebagai Driver Utama (Proxy Ketiadaan Data Sulteng)
    driver_tambang = data_empiris.get('deforestasi_driver_tambang_ha', 0.0) # (513k Sultra * 2)
    skor_lahan_4 = min(10.0, (driver_tambang / 500_000.0) * 10.0)
    
    # 3E. Kepadatan Izin Tambang (Total IUP vs Luas Daratan)
    rasio_ekspansi = data_empiris.get('rasio_gap_amdal_iup', 0.0) 
    # Normalisasi: Jika >10% wilayah daratan dikavling IUP = Skor 10 (Kritis)
    skor_lahan_5 = min(10.0, (rasio_ekspansi / 0.10) * 10.0)
    ...
    # ---------------------------------------------------------
    izin_baru = data_empiris.get('jumlah_izin_baru_krisis', 0)
    skor_veto_1 = min(10.0, (izin_baru / 100.0) * 10.0)
    
    izin_ilegal = data_empiris.get('perusahaan_ilegal_pemutihan', 0)
    skor_veto_2 = min(10.0, (izin_ilegal / 10.0) * 10.0)
    
    pltu_captive = data_empiris.get('kapasitas_pltu_mw', 0)
