...
def load_data():
    ...
    return df_kes, df_ika, df_bencana, df_konflik, df_izin, df_iku, df_b3, df_pltu_op, df_gfw, df_gfw_lindung, df_gfw_driver

df_kes, df_ika, df_bencana, df_konflik, df_izin, df_iku, df_b3, df_pltu_op, df_gfw, df_gfw_lindung, df_gfw_driver = load_data()
...
if not df_konflik.empty:
    keywords = 'air|laut|pesisir|nelayan|sungai|pulau|tailing'
    df_konflik_darat = df_konflik[~df_konflik['sektor'].str.contains(keywords, case=False, na=False)]
    konflik_darat = len(df_konflik_darat)
    skor_sosial_1 = min(10.0, (konflik_darat / 300) * 10)

skor_akumulasi_sosial = skor_sosial_1

skor_veto_1 = 0
luas_izin_sentra = 0
if not df_izin.empty:
    df_izin_sentra = df_izin[df_izin['Provinsi'].isin(['Sulawesi Tengah', 'Sulawesi Tenggara'])].copy()
    df_izin_sentra['Total_Luas_Konsesi_Baru_Ha'] = pd.to_numeric(df_izin_sentra['Total_Luas_Konsesi_Baru_Ha'], errors='coerce').fillna(0)
    luas_izin_sentra = df_izin_sentra['Total_Luas_Konsesi_Baru_Ha'].sum()
    skor_veto_1 = min(10.0, (luas_izin_sentra / 500_000) * 10)

skor_akumulasi_veto = skor_veto_1
