...
st.set_page_config(page_title="Beban Kesehatan Masyarakat Terdampak — CELIOS ECC", layout="wide")
render_sidebar()
...
def load_health_data():
    ...
    return df_kes, df_faskes, df_iku, df_pltu

df_kes, df_faskes, df_iku, df_pltu = load_health_data()
...
st.markdown('<div class="main-title">Beban Kesehatan Masyarakat Terdampak</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">Membongkar tragedi kesehatan publik akibat paparan emisi dan polutan industri di kawasan penyangga smelter nikel Sulawesi.</div>', unsafe_allow_html=True)
...
st.markdown('<span style="background:#5C2B6A;color:#E1BEE7;padding:4px 10px;border-radius:5px;font-size:0.85rem;">Metode: Comparative Spatial Analysis (Dinas Kesehatan 2014-2024)</span>', unsafe_allow_html=True)
...
df_kes_copy['Kategori'] = df_kes_copy['provinsi'].apply(lambda x: 'Sentra Industri (Sulteng & Sultra)' if x in sentra else 'Non-Sentra Industri (Sulsel, Sulut, Gorontalo, Sulbar)')
...
fig_3_1.update_layout(
    ...
    yaxis=dict(title="Rata-Rata Kasus per Tahun", showgrid=True, gridcolor='rgba(255,255,255,0.1)')
)
...
with st.expander("Lihat Data Mentah: Komparasi Kasus per Provinsi", expanded=False):
    ...
    st.caption("📁 **Sumber File:** `data/processed/sulawesi_kesehatan_detail_2014_2024.csv` - Agregasi Rata-rata per Kategori Wilayah")
...
with col_sel1:
    ...
    x_options = {
        ...
        "IKU_Point_Alt": "Skor IKU (Alternatif)"
    }
    x_col = st.selectbox("Pilih Indikator Lingkungan (X):", list(x_options.keys()), format_func=lambda x: x_options[x])
...
with col_sel2:
    ...
    y_col = st.selectbox("Pilih Indikator Penyakit (Y):", list(y_options.keys()), format_func=lambda x: y_options[x])
...
with st.expander("Lihat Data Panel Mentah (Merge Izin & Dinas Kesehatan)", expanded=False):
    st.dataframe(df_panel[['Provinsi', 'Tahun', x_col, 'X_Label', y_col, 'Y_Label']], use_container_width=True, hide_index=True)
    st.caption("Sumber: Gabungan `sulawesi_iku_2015_2024.csv` (KLHK) dan `sulawesi_kesehatan_detail_2014_2024.csv` (Dinas Kesehatan).")
...
if geojson_data:
    ...
    max_val = max(df_map_2015['Kasus ISPA/Pneumonia'].max(), df_map_2024['Kasus ISPA/Pneumonia'].max())
    min_val = min(df_map_2015['Kasus ISPA/Pneumonia'].min(), df_map_2024['Kasus ISPA/Pneumonia'].min())
    diff = max_val - min_val
    fixed_bins = [min_val, min_val + diff*0.2, min_val + diff*0.4, min_val + diff*0.6, min_val + diff*0.8, max_val]
    ...
    with col_map1:
        ...
        st_folium(m_2015, use_container_width=True, height=500, returned_objects=[], key="map_2015")
    ...
    with col_map2:
        ...
        st_folium(m_2024, use_container_width=True, height=500, returned_objects=[], key="map_2024")
...
# ══════════════════════════════════════════════════════════
st.markdown("---")
st.markdown('<h2 style="color: #ECEFF1; font-size: 24px;">3.5 Krisis Air Bersih: Penurunan IKA & Ledakan Kasus Diare</h2>', unsafe_allow_html=True)
st.markdown('<span style="background:#00695C;color:#B2DFDB;padding:4px 10px;border-radius:5px;font-size:0.85rem;">Metode: Panel Crosstab Analysis (IKA × Diare, 2016-2024)</span>', unsafe_allow_html=True)
...
df_ika.columns = ['Tahun', 'Provinsi', 'IKA', 'Sumber']

# --- Merge with Diare Data ---
df_diare_only = df_kes[df_kes['indikator'] == 'Kasus Diare Dilayani'][['provinsi', 'tahun', 'nilai']].copy()
...
df_ika_diare['Kategori'] = df_ika_diare['Provinsi'].apply(
    lambda x: 'Sentra Industri (Sulteng & Sultra)' if x in sentra_industri else 'Non-Sentra Industri (Lainnya)'
)
...
df_ika_diare['Year_Normalized'] = ((df_ika_diare['Tahun'] - df_ika_diare['Tahun'].min()) / 
                                     (df_ika_diare['Tahun'].max() - df_ika_diare['Tahun'].min())) * 20 + 8
...
fig_34_scatter = px.scatter(
    ...
    labels={
        ...
        'Total_Diare': 'Kasus Diare per Tahun'
    }
)
...
ika_sentra_mean = df_ika_diare[df_ika_diare['Kategori'].str.contains('Sentra')]['IKA'].mean()
ika_non_mean = df_ika_diare[~df_ika_diare['Kategori'].str.contains('Sentra')]['IKA'].mean()
diare_sentra_mean = df_ika_diare[df_ika_diare['Kategori'].str.contains('Sentra')]['Total_Diare'].mean()
diare_non_mean = df_ika_diare[~df_ika_diare['Kategori'].str.contains('Sentra')]['Total_Diare'].mean()

# Interpretation based on regression
correlation_strength = "sangat kuat" if abs(r_squared) > 0.5 else "kuat" if abs(r_squared) > 0.3 else "moderat" if abs(r_squared) > 0.1 else "lemah"
...
with col_sel_ika1:
    ...
    x_col_ika = st.selectbox("Pilih Indikator Lingkungan (X):", list(x_options_ika.keys()), format_func=lambda x: x_options_ika[x], key="x_ika_select")
...
with col_sel_ika2:
    ...
    y_col_ika = st.selectbox("Pilih Indikator Penyakit (Y):", list(y_options_ika.keys()), format_func=lambda x: y_options_ika[x], key="y_ika_select")
...
df_ika_diare["IKA_Label"] = df_ika_diare['IKA'].apply(lambda x: label_ika_high if x >= ika_median else label_ika_low)
df_ika_diare["Diare_Label"] = df_ika_diare['Total_Diare'].apply(lambda x: label_diare_high if x >= diare_median else label_diare_low)

# Crosstab
cats_ika = [label_ika_low, label_ika_high]
cats_diare = [label_diare_low, label_diare_high]
crosstab_ika = pd.crosstab(df_ika_diare["IKA_Label"], df_ika_diare["Diare_Label"]).reindex(index=cats_ika, columns=cats_diare, fill_value=0)

chi2_ika, p_ika, dof_ika, expected_ika = stats.chi2_contingency(crosstab_ika)
expected_ika_df = pd.DataFrame(expected_ika, index=crosstab_ika.index, columns=crosstab_ika.columns)
...
columns_case_ika = pd.MultiIndex.from_product([["Cases"], ["Valid", "Missing", "Total"], ["N", "Percent"]])
interaction_label_ika = "Indeks Kualitas Air (IKA) * Kasus Diare"
row_data_ika = [
    valid_cases_ika, f"{valid_cases_ika/total_cases_ika*100:.1f}%",
    missing_cases_ika, f"{missing_cases_ika/total_cases_ika*100:.1f}%",
    total_cases_ika, "100.0%"
]
case_summary_ika = pd.DataFrame([row_data_ika], index=[interaction_label_ika], columns=columns_case_ika)
...
multi_index_ika = pd.MultiIndex.from_tuples(row_indices_ika, names=["Indeks Kualitas Air", ""])
spss_crosstab_ika = pd.DataFrame(rows_ika, index=multi_index_ika, columns=cats_diare + ["Total"])
...
chi_df_ika = pd.DataFrame(chi_data_ika, index=["Pearson Chi-Square", "Likelihood Ratio", "Linear-by-Linear Association", "N of Valid Cases"], columns=["Value", "df", "Asymp. Sig. (2-sided)"])
...
status_text_ika = "SIGNIFIKAN (Ada Hubungan)" if is_significant_ika else "TIDAK SIGNIFIKAN"
order_color_ika = "#4CAF50" if is_significant_ika else "#F44336"
bg_color_ika = "rgba(76, 175, 80, 0.1)" if is_significant_ika else "rgba(244, 67, 54, 0.1)"
...
with st.expander("Lihat Data Panel: IKA × Diare (2016-2024)", expanded=False):
    df_ika_diare_display = df_ika_diare[['Tahun', 'Provinsi', 'IKA', 'Total_Diare', 'Kategori', 'IKA_Label', 'Diare_Label']].copy()
    st.dataframe(df_ika_diare_display, use_container_width=True, hide_index=True)
    st.caption("📁 **Sumber File:** `data/processed/sulawesi_ika_2016_2024.csv` + `data/processed/sulawesi_kesehatan_detail_2014_2024.csv`")

# --- Executive Summary of All Combinations ---
st.markdown("---")
st.markdown("### Ringkasan Eksekutif Seluruh Skenario Crosstab")
st.markdown("Tabel di bawah ini merangkum hasil pengujian statistik (Chi-Square) untuk indikator IKA terhadap Kasus Diare pada panel data yang sama.")
...
s_ika_x = df_ika_diare['IKA'].apply(lambda val: lbl_ika_h if val >= med_ika_x else lbl_ika_l)
s_diare_y = df_ika_diare['Total_Diare'].apply(lambda val: lbl_diare_h if val >= med_diare_y else lbl_diare_l)

ct_ika = pd.crosstab(s_ika_x, s_diare_y).reindex(index=[lbl_ika_l, lbl_ika_h], columns=[lbl_diare_l, lbl_diare_h], fill_value=0)
try:
...
# Generate Dynamic Narrative for Executive Summary
sig_count_ika = sum(1 for row in summary_data_ika if "🟢 SIGNIFIKAN" in row["Kesimpulan"])
...
# ══════════════════════════════════════════════════════════
st.markdown("---")
st.markdown('<h2 style="color: #ECEFF1; font-size: 24px;">3.6 Beban Limbah Beracun (B3): Eksternalitas Kesehatan yang Diabaikan</h2>', unsafe_allow_html=True)
st.markdown('<span style="background:#BF360C;color:#FFCCBC;padding:4px 10px;border-radius:5px;font-size:0.85rem;">Metode: Descriptive Statistics & Comparative Bar Chart (2020-2024)</span>', unsafe_allow_html=True)
...
df_b3_agg = df_b3[df_b3['Estimasi Timbulan (Ton/Tahun)'] > 1000].copy()  # Filter only major sources with >1000 tons

# Aggregate by Province and Waste Type
df_b3_by_prov = df_b3_agg.groupby('Provinsi')['Estimasi Timbulan (Ton/Tahun)'].sum().reset_index()
df_b3_by_prov = df_b3_by_prov.sort_values('Estimasi Timbulan (Ton/Tahun)', ascending=False)
...
sulteng_b3 = df_b3_by_prov[df_b3_by_prov['Provinsi'] == 'Sulawesi Tengah']['Estimasi Timbulan (Ton/Tahun)'].values[0] if 'Sulawesi Tengah' in df_b3_by_prov['Provinsi'].values else 0
sultra_b3 = df_b3_by_prov[df_b3_by_prov['Provinsi'] == 'Sulawesi Tenggara']['Estimasi Timbulan (Ton/Tahun)'].values[0] if 'Sulawesi Tenggara' in df_b3_by_prov['Provinsi'].values else 0
...
slag_total = df_b3_by_type[df_b3_by_type['Jenis Limbah B3'].str.contains('Slag', case=False, na=False)]['Estimasi Timbulan (Ton/Tahun)'].sum()
tailing_total = df_b3_by_type[df_b3_by_type['Jenis Limbah B3'].str.contains('Tailing', case=False, na=False)]['Estimasi Timbulan (Ton/Tahun)'].sum()
...
df_b3_facilities = df_b3_agg[['Provinsi', 'Kawasan/Perusahaan', 'Jenis Limbah B3', 'Estimasi Timbulan (Ton/Tahun)', 'Sumber Referensi']].copy()
df_b3_facilities = df_b3_facilities.sort_values('Estimasi Timbulan (Ton/Tahun)', ascending=False).head(10)
df_b3_facilities['Estimasi Timbulan (Ton/Tahun)'] = df_b3_facilities['Estimasi Timbulan (Ton/Tahun)'].apply(lambda x: f"{x:,.0f}")
...
st.dataframe(
    ...
    column_config={
        ...
        "Kawasan/Perusahaan": st.column_config.TextColumn("Kawasan/Perusahaan", width="large"),
        "Jenis Limbah B3": st.column_config.TextColumn("Jenis Limbah", width="medium"),
        "Estimasi Timbulan (Ton/Tahun)": st.column_config.TextColumn("Timbulan (Ton/Tahun)", width="medium"),
        "Sumber Referensi": st.column_config.TextColumn("Sumber", width="large")
    }
)

st.caption("**Top 10 Fasilitas Penghasil Limbah B3** — Data dikompilasi dari laporan AEER (2024), WALHI, JATAM, dan audit lingkungan independen.")
...
with st.expander("Lihat Data Mentah: Limbah B3 Sulawesi (2020-2024)", expanded=False):
    ...
    st.caption("📁 **Sumber File:** `data/processed/sulawesi_limbah_b3.csv` — Kompilasi dari AEER Report (2024), WALHI, JATAM, BPLH, dan kajian akademis independen.")
