...
| **Sosial 4 (Faskes)** | Di-*hardcode* manual `spa_aktual_pct = 42.5`. Menghitung Gap Target SPA 80%. | Awalnya melanggar pakem *(hardcode)* dengan mengubah metrik menjadi "Rasio Pasien per Faskes". | **UI**: 42.5% (Hardcode statis).<br>**Algo**: *Sudah direvert* kembali menghitung Gap Target SPA 80%. | **UI Sangat Benar.** (Algoritma Provinsi sebelumnya sempat melenceng dari desain dokumen dengan mengukur rasio pasien. Sudah dikoreksi agar kembali patuh pada threshold SPA 80%). |
...
3. **Sosial 4 (Faskes):** Kesalahan fatal pada Backend (Provinsi) yang sempat mengubah metrik diam-diam menjadi "Rasio Pasien per Faskes" sudah di-revert/dikembalikan ke jalan yang benar. Keduanya kini sudah 100% menggunakan indikator **Gap Target SPA 80%**.
