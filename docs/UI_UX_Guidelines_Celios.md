...
*   **Struktur Header Eksekutif:** Di bagian atas setiap halaman analisis penting, wajib menggunakan urutan **Dropdown Metodologi**, **Hero Statement**, lalu **Bento Cards**.
    ...
    2.  **Hero Statement (Paragraf Narasi Kritis Utama):** Ini adalah teks pengantar (tanpa kotak aksen/warna terang, cukup *background* transparan) yang diletakkan tepat di bawah Dropdown Metodologi. Paragraf ini **WAJIB** dibangun sebagai sebuah *storytelling* komprehensif yang merangkai dan menceritakan makna dari **seluruh** angka temuan kritis yang akan ditampilkan di kartu-kartu bawahnya. Jangan membiarkan angka terlepas dari konteks narasi utamanya.
...
*   **Kotak Interpretasi/Pembedahan Realitas Ekologis:** Di bawah setiap visualisasi (grafik) atau tabel eksekutif, wajib ada ringkasan narasi kritis.
    *   *Standar Ketajaman Narasi (Data-Driven >250 Kata):* Khusus untuk bagian *dashboard* yang membedah krisis (seperti "Pembedahan Ekologis"), narasi deskripsi tidak boleh pendek atau pasif. Anda **WAJIB** membuat ulasan jurnalistik-kritis yang panjang (minimal 250 kata) dan *data-driven*. Narasi ini harus mengekstrak angka mutlak dari visualisasi (menggunakan Python `f-strings`), menyebut langsung total hektar atau emisi secara eksplisit, dan menarik kesimpulan kausalitas yang membantah narasi arus utama (misalnya "Membantah Hilirisasi Hijau").
    ...
    *   *Styling Variabel Dinamis (F-String):* Setiap angka/variabel dinamis (seperti `{total_kasus}`) yang dimasukkan ke dalam narasi `<div>` **WAJIB** di-*highlight* menggunakan *inline CSS span* dengan warna kontras (contoh: `<span style="color:#E53935;font-weight:bold;">{total_kasus}</span>`). Jangan hanya menggunakan tag `<b>`, karena di beberapa tema Streamlit teks bold tidak cukup menonjol dari teks biasa. Tanpa warna dan *font-weight* ini, variabel dinamis akan mati dan menyatu dengan teks biasa, menghilangkan esensi keterbacaan data.
